function [f, inlierIdx] = ransac1( x,y,ransacCoef,funcFindF,funcDist )
    ...
    %   Here, M = 2, i.e. the points are on two-dimesnional space. 
    ...
    minPtNum = ransacCoef.minPtNum;                 %Minimum number of points required to solve for homography matrix
    iterNum = ransacCoef.iterNum;                   %Maximum number of iterations allowed in the algorithm
    thInlrRatio = ransacCoef.thInlrRatio;           %Multiplied by total number of points, gives the number of close data points required to assert that a model fits well to data
    thDist = ransacCoef.thDist;                     %Threshold value to determine when a data point fits a model
    ptNum = size(x,2);                              %Number of points in matchLoc2
    thInlr = round(thInlrRatio*ptNum);              %No. of close data points required to assert that a model fits well to data
