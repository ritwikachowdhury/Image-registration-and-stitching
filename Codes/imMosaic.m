%Actual function for mosaicing two images
function [imgout] = imMosaic(img1, img2, adjColor)
    ...
    %	If you have more than 2 images to do mosaic, call this function several times.
    %	If you set adjColor to 1, imMosaic will try to adjust the
    ...
    % Applying transformation to the second image
    tform = projective2d(H');
    img21 = imwarp(img2,tform);
    figure,imshow(img1)
    figure,imshow(img21)

    % Linear blending for the 2 images
    [M1 ,N1, dim] = size(img1);
    [M2 ,N2, ~] = size(img2);
    ...
    % Transformed coordinates for the corner points of an image stored in pt
    pt = zeros(3,4);
    pt(:,1) = H*[1;1;1];
    pt(:,2) = H*[N2;1;1];
    pt(:,3) = H*[N2;M2;1];
